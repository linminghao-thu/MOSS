...
class GenericEvaluator:
    ...
    def clean(self,s,ifreader = False):
        ...
        if ifreader:
            ...
            s = '<GO> ' + s + ' </s>'
        slot_list = []
        assert(len(self.entity_dict)>0)
        for item in self.entity_dict:
            
            # s = s.replace(item, 'VALUE_{}'.format(self.entity_dict[item]))
            s1 = clean_replace(s, item, '{}_SLOT'.format(self.entity_dict[item]))
            if s1 != s:
...
class CamRestEvaluator(GenericEvaluator):
    ...
    def run_metrics(self):
        ...
        for i, row in enumerate(data):
            XXX = data[i]['response']
            data[i]['response'],slot_list1 = self.clean(data[i]['response'])
            laoXXX = data[i]['generated_response']
            data[i]['generated_response'],slot_list2 = self.clean(data[i]['generated_response'])
            # if len(slot_list1) > 0:
            #     print("XXX: ",XXX)
            #     print("response list: ",slot_list1)
            # if len(slot_list2) > 0:
            #     print("laoXXX: ",laoXXX )
            #     print("generated response: ",slot_list2)
        bleu_score = self.bleu_metric(data,'bleu')
        print("bleu: ",bleu_score)
        z3_success_f1 = self.z3_success_f1_metric(data, 'success', 'generated_system')
        print("generated_system - response： z3_success_f1: ", z3_success_f1)
        z3_success_f1 = self.z3_success_f1_metric(data, 'success', 'system')
        print("system - response: z3_success_f1: ", z3_success_f1)
        z3_success_f1 = self.z3_success_f1_metric(data, 'success', 'generated_system', 'system')
        print("generated_system - system： z3_success_f1: ", z3_success_f1)
    ...
    @report
    def z3_success_f1_metric(self, data, sub='successf1', src_in = 'generated_system', src_out = 'response'):
        dials = self.pack_dial(data)
        tp,fp,fn = 0,0,0
        for dial_id in dials:
            ...
            dial = dials[dial_id]
            for turn_num, turn in enumerate(dial):
                if src_in is "response":
                    gen_response_token = turn[src_in].split()
                else:
                    cleaned_gen_res, _ = self.clean(turn[src_in])
                    gen_response_token = cleaned_gen_res.split()
                
                 
                
                if src_out is "response":
                    response_token = turn[src_out].split()
                else:
                    cleaned_res, _ = self.clean(turn[src_out])
                    response_token = cleaned_res.split()
