...
class BSpanDecoder_noCopyNet(nn.Module):
    def __init__(self, bspanEmb, embed_size, hidden_size, vocab_size, dropout_rate, vocab):
        super().__init__()
        self.emb = bspanEmb #nn.Embedding(vocab_size, embed_size)
    ...
    def forward(self, u_enc_out, z_tm1, last_hidden, u_input_np, pv_z_enc_out, prev_z_input_np, u_emb, pv_z_emb,
        ...
        if cfg.use_positional_embedding:  # defaulty not used
            ...
            raise NotImplementedError("Position Embedding.")
        ...
        scores = F.softmax(gen_score, dim=1)
       
        gen_score = scores[:, :cfg.vocab_size]
...
class TSD(nn.Module):
    def __init__(self, embed_size, hidden_size, vocab_size, degree_size, layer_num, dropout_rate, z_length,
        ...
        self.z1_decoder = BSpanDecoder(self.emb, embed_size, hidden_size, vocab_size, dropout_rate, self.vocab)
       
        self.z3_decoder = BSpanDecoder_noCopyNet(self.emb, embed_size, hidden_size, vocab_size, dropout_rate, self.vocab)
        ...
        if cfg.train_with_weight is False: 
            self.pr_loss = nn.NLLLoss(ignore_index=0, weight=None)
            self.pr1_loss = nn.NLLLoss(ignore_index=0, weight=None)
            self.pr2_loss = nn.NLLLoss(ignore_index=0, weight=None)
            self.pr3_loss = nn.NLLLoss(ignore_index=0, weight=None)
        else:    
            from class_weight import get_weight_dict
            from class_weight import my_tfidf
            

            weight_dict = my_tfidf()
            
            weight_np = np.ones(100)
            for key, value in weight_dict.items():
                wrd_idx = self.vocab.encode(key)
                weight_np[wrd_idx] = value
            

            
            weight = torch.from_numpy(weight_np).float()
            
            self.pr_loss = nn.NLLLoss(ignore_index=0, weight=weight)
            self.pr1_loss = nn.NLLLoss(ignore_index=0, weight=weight)
            self.pr2_loss = nn.NLLLoss(ignore_index=0, weight=weight)
            self.pr3_loss = nn.NLLLoss(ignore_index=0, weight=weight)
        ...
        #  added
        self.hidden_size = hidden_size
        bert_hidden_size = 768 # hard coded 

        
        

    def forward(self, u_input, u_input_np, m_input, m_input_np, z_input, z1_input, z2_input, z3_input, u_len, m_len, turn_states,
                degree_input, mode, **kwargs):
        if mode == 'train' or mode == 'valid':
            pz_proba, pz1_proba, pz2_proba, pz3_proba, pm_dec_proba, turn_states = \
                self.forward_turn(u_input, u_len, m_input=m_input, m_len=m_len, z_input=z_input, z1_input=z1_input, z2_input=z2_input, z3_input=z3_input,  mode='train',
                                  turn_states=turn_states, degree_input=degree_input, u_input_np=u_input_np,
                                  m_input_np=m_input_np, **kwargs)
            loss, pr_loss,pr1_loss,pr2_loss,pr3_loss, m_loss = self.supervised_loss(torch.log(pz_proba), torch.log(pz1_proba), torch.log(pz2_proba), torch.log(pz3_proba), torch.log(pm_dec_proba),
                                                         z_input, z1_input, z2_input, z3_input,  m_input)
            #print(loss)
            return loss, pr_loss,pr1_loss,pr2_loss,pr3_loss, m_loss, turn_states

        elif mode == 'test':
            m_output_index, pz_index, pz1_index, pz2_index, pz3_index, turn_states = self.forward_turn(u_input, u_len=u_len, mode='test',
                                                                      turn_states=turn_states,
                                                                      degree_input=degree_input,
                                                                      u_input_np=u_input_np, m_input_np=m_input_np,
                                                                      **kwargs
                                                                      )
            return m_output_index, pz_index, pz1_index, pz2_index, pz3_index, turn_states
        elif mode == 'rl':
    ...
    def forward_turn(self, u_input, u_len, turn_states, mode, degree_input, u_input_np, m_input_np=None,
                     m_input=None, m_len=None, z_input=None, z1_input=None, z2_input=None, z3_input=None, **kwargs):
        ...
        assert(prev_z_input_np is not None)
        ...
        prev_z1_input = kwargs.get('prev_z1_input', None)
        prev_z1_input_np = kwargs.get('prev_z1_input_np', None)
        prev_z1_len = kwargs.get('prev_z1_len', None)
        pv_z1_emb = None
        # pv_z1_enc_out = None

        prev_z2_input = kwargs.get('prev_z2_input', None)
        prev_z2_input_np = kwargs.get('prev_z2_input_np', None)
        prev_z2_len = kwargs.get('prev_z2_len', None)
        pv_z2_emb = None
        # pv_z2_enc_out = None

        prev_z3_input = kwargs.get('prev_z3_input', None)
        prev_z3_input_np = kwargs.get('prev_z3_input_np', None)
        prev_z3_len = kwargs.get('prev_z3_len', None)
        pv_z3_emb = None
        # pv_z3_enc_out = None
        ...
        last_hidden_bypass_z = last_hidden


        z_tm1 = cuda_(Variable(torch.ones(1, batch_size).long() * 3))  # GO_2 token
        
        z1_tm1 = cuda_(Variable(torch.ones(1, batch_size).long() * 3))  # GO_2 token
        z2_tm1 = cuda_(Variable(torch.ones(1, batch_size).long() * 3))  # GO_2 token
        z3_tm1 = cuda_(Variable(torch.ones(1, batch_size).long() * 3))  # GO_2 token
        ...
        if mode == 'train':
            ...
            last_hidden = last_hidden_bypass_z
            ...
            z1_length = z1_input.size(0) if z1_input is not None else self.z_length  # GO token
            z1_hiddens = [None] * batch_size
            z2_length = z2_input.size(0) if z2_input is not None else self.z_length  # GO token
            z2_hiddens = [None] * batch_size
            z3_length = z3_input.size(0) if z3_input is not None else self.z_length  # GO token
            z3_hiddens = [None] * batch_size

            
            for t in range(z1_length):
                pz1_dec_out, last_hidden, proba = \
                    self.z1_decoder(u_enc_out=u_enc_out, u_input_np=u_input_np,
                                   z_tm1=z1_tm1, last_hidden=last_hidden,
                                   pv_z_enc_out=pv_z_enc_out, prev_z_input_np=prev_z_input_np,
                                   u_emb=u_emb, pv_z_emb=pv_z1_emb, position=t)
                pz1_proba.append(proba)
                pz1_dec_outs.append(pz1_dec_out)
                z1_np = z1_tm1.view(-1).cpu().data.numpy()
                for i in range(batch_size):
                    if z1_np[i] == self.vocab.encode('EOS_Z2'):
                        hiddens[i] = last_hidden[:, i, :]
                z1_tm1 = z1_input[t].view(1, -1)
            ...
            z1_input_np = z1_input.cpu().data.numpy()
            pz1_dec_outs = torch.cat(pz1_dec_outs, dim=0)  # [Tz,B,H]
            pz1_proba = torch.stack(pz1_proba, dim=0)
            
          

            pz_dec_outs = pz1_dec_outs 
            z_input_np = z1_input_np 
            

            for t in range(z3_length):
                stop_tok = [self.vocab.encode('EOS_Z2')]
                pz3_dec_out, last_hidden, proba = \
                    self.z3_decoder(u_enc_out=pz_dec_outs, u_input_np=z_input_np,
                                   z_tm1=z3_tm1, last_hidden=last_hidden,
                                   pv_z_enc_out=pv_z_enc_out, prev_z_input_np=prev_z_input_np,
                                   u_emb=u_emb, pv_z_emb=pv_z3_emb, position=t,
                                   stop_tok = stop_tok)
                pz3_proba.append(proba)
                pz3_dec_outs.append(pz3_dec_out)
                z3_np = z3_tm1.view(-1).cpu().data.numpy()
                for i in range(batch_size):
                    if z3_np[i] == self.vocab.encode('<split>'):
                        hiddens[i] = last_hidden[:, i, :]
                z3_tm1 = z3_input[t].view(1, -1)

            for i in range(batch_size):
                if hiddens[i] is None:
                    hiddens[i] = last_hidden[:, i, :]
            last_hidden = torch.stack(hiddens, dim=1)
            z3_input_np = z3_input.cpu().data.numpy()
            pz3_dec_outs = torch.cat(pz3_dec_outs, dim=0)  
            pz3_proba = torch.stack(pz3_proba, dim=0)
            ...
            pz_proba = pz1_proba
            pz2_proba = pz1_proba
            return pz_proba, pz1_proba, pz2_proba, pz3_proba, pm_dec_proba, None
        else:

            last_hidden = last_hidden_bypass_z

            pz1_dec_outs, bspan1_index, last_hidden = self.bspan1_decoder(u_enc_out, z1_tm1, last_hidden, u_input_np,
                                                                       pv_z_enc_out=pv_z_enc_out,
                                                                       prev_z_input_np=prev_z_input_np,
                                                                       u_emb=u_emb, pv_z_emb=pv_z1_emb)
            
            pz1_dec_outs = torch.cat(pz1_dec_outs, dim=0)
            
          
            
            pz_dec_outs = pz1_dec_outs #torch.cat([pz1_dec_outs, pz2_dec_outs], 0) 
            bspan_index = np.array(bspan1_index) #np.concatenate((bspan1_index, bspan2_index), 1)
            #print("bspan1_index.shape", bspan1_index.shape)
            #print("bspan2_index.shape", bspan2_index.shape)
            bspan_index_transpose = bspan_index.transpose(1,0)
            #print("bspan_index.shape", bspan_index.shape)
            #print("prev_z_input_np.shape",prev_z_input_np.shape)
            #print("bspan_index_transpose.shape",bspan_index_transpose.shape)


            stop_tok = [self.vocab.encode('EOS_Z2')]
            #  u_enc_out=pz_dec_outs, u_input_np=z_input_np bspan_index
            pz3_dec_outs, bspan3_index, last_hidden = self.bspan3_decoder(pz_dec_outs, z3_tm1, last_hidden, bspan_index_transpose,
                                                                       pv_z_enc_out=pv_z_enc_out,
                                                                       prev_z_input_np=prev_z_input_np,
                                                                       u_emb=u_emb, pv_z_emb=pv_z3_emb, stop_tok=stop_tok)
            



            #print("pz_dec_outs: ",pz_dec_outs.shape )
            #bspan_index1 = torch.cat(bspan_index,dim=0)
            #print("bspan_inde1: ",bspan_index.shape)
                
            #pz_dec_outs = torch.cat(pz_dec_outs, dim=0)
            pz3_dec_outs = torch.cat(pz3_dec_outs, dim=0)
            degree_input = self.reader.db_degree_handler(bspan1_index, kwargs['dial_id'])
            
            # of no usr
            #degree_input = self.reader.db_degree_handler(bspan3_index, kwargs['dial_id'])



            #degree_input = np.array([0,0,0,1,0])
            ...
            if mode == 'test':
                if not self.beam_search:
                    ...
                    # u_enc_out --> pz3_dec_outs,  u_input_np-->z3_input_np bspan3_index
                    
                    #m_output_index = self.greedy_decode(pz_dec_outs, pz3_dec_outs, m_tm1, bspan3_index, last_hidden,
                    #                                    degree_input, bspan_index)
                    m_output_index = self.greedy_decode(pz_dec_outs, u_enc_out, m_tm1, u_input_np, last_hidden,
                                                        degree_input, bspan_index)
                    ...
                    #                                         degree_input, bspan_index)
                    # u_enc_out --> pz3_dec_out,  u_input_np-->z3_input_np bspan3_index
                    m_output_index = self.beam_search_decode(pz_dec_outs, u_enc_out, m_tm1, u_input_np, last_hidden,
                                                             degree_input, bspan_index)
                    
                bspan_index = bspan1_index
                bspan2_index = bspan1_index
                return m_output_index, bspan_index, bspan1_index, bspan2_index, bspan3_index, None
            elif mode == 'rl':
                # u_enc_out --> pz3_dec_out,  u_input_np-->z3_input_np bspan3_index
    ...
    def bspan_decoder(self, u_enc_out, z_tm1, last_hidden, u_input_np, pv_z_enc_out, prev_z_input_np, u_emb, pv_z_emb):
        ...
        pz_proba = []
        decoded = []
        batch_size = pv_z_enc_out.size(1)
        hiddens = [None] * batch_size
    ...
    def bspan1_decoder(self, u_enc_out, z_tm1, last_hidden, u_input_np, pv_z_enc_out, prev_z_input_np, u_emb, pv_z_emb):
        pz_dec_outs = []
        pz_proba = []
        decoded = []
        batch_size = pv_z_enc_out.size(1)
    ...
    def bspan2_decoder(self, u_enc_out, z_tm1, last_hidden, u_input_np, pv_z_enc_out, prev_z_input_np, u_emb, pv_z_emb):
        pz_dec_outs = []
        pz_proba = []
        decoded = []
        batch_size = pv_z_enc_out.size(1)
        ...
        for t in range(cfg.z_length):
            pz_dec_out, last_hidden, proba = \
                self.z2_decoder(u_enc_out=u_enc_out, u_input_np=u_input_np,
                               z_tm1=z_tm1, last_hidden=last_hidden, pv_z_enc_out=pv_z_enc_out,
                               prev_z_input_np=prev_z_input_np, u_emb=u_emb, pv_z_emb=pv_z_emb, position=t)
            pz_proba.append(proba)
            pz_dec_outs.append(pz_dec_out)
    ...
    def bspan3_decoder(self, u_enc_out, z_tm1, last_hidden, u_input_np, pv_z_enc_out, prev_z_input_np, u_emb, pv_z_emb, stop_tok=None):
        pz_dec_outs = []
        pz_proba = []
        decoded = []
        batch_size = pv_z_enc_out.size(1)
        ...
        for t in range(cfg.z_length):
            pz_dec_out, last_hidden, proba = \
                self.z3_decoder(u_enc_out=u_enc_out, u_input_np=u_input_np,
                               z_tm1=z_tm1, last_hidden=last_hidden, pv_z_enc_out=pv_z_enc_out,
                               prev_z_input_np=prev_z_input_np, u_emb=u_emb, pv_z_emb=pv_z_emb, position=t,
                               stop_tok=stop_tok)
            pz_proba.append(proba)
            ...
            z_np = z_tm1.view(-1).cpu().data.numpy()
            for i in range(batch_size):
                if z_np[i] == self.vocab.encode('<split>'):
                    hiddens[i] = last_hidden[:, i, :]
            z_tm1 = cuda_(Variable(z_index).view(1, -1))
        for i in range(batch_size):
            if hiddens[i] is None:
    ...
    def supervised_loss(self, pz_proba, pz1_proba, pz2_proba, pz3_proba, pm_dec_proba, z_input,z1_input,z2_input,z3_input, m_input):
        ...
        pz1_proba = pz1_proba[:, :, :cfg.vocab_size].contiguous() 
        pz2_proba = pz2_proba[:, :, :cfg.vocab_size].contiguous() 
        pz3_proba = pz3_proba[:, :, :cfg.vocab_size].contiguous() 
        pr1_loss = self.pr1_loss(pz1_proba.view(-1, pz1_proba.size(2)), z1_input.view(-1))
        # pr2_loss = self.pr2_loss(pz2_proba.view(-1, pz2_proba.size(2)), z2_input.view(-1))
        pr3_loss = self.pr3_loss(pz3_proba.view(-1, pz3_proba.size(2)), z3_input.view(-1))
        

        #print("pm_dec_proba.size()", pm_dec_proba.size())
        #print("m_input.size()", m_input.size() )
        
        m_loss = self.dec_loss(pm_dec_proba.view(-1, pm_dec_proba.size(2)), m_input.view(-1))


        # only x plus y
        # loss = m_loss + pr1_loss + pr2_loss + pr3_loss  #  * 5
        loss = m_loss + pr1_loss + pr3_loss  #  * 5
        ...
        pr_loss = pr1_loss
        pr2_loss = pr1_loss
        return loss, pr_loss, pr1_loss, pr2_loss, pr3_loss, m_loss
