...
class _ReaderBase:
    ...
    class Vocab(LabelSet):
        ...
        def decode(self, idx):
            ...
            if type(idx) == torch.Tensor:
                idx1 = idx.cpu().numpy().item()
            if idx1 < len(self):
                return self._idx2item[idx1]
            else:
                return 'ITEM_%d' % (idx1 - cfg.vocab_size)
    ...
    def wrap_result(self, turn_batch, gen_m, gen_z,gen_z1,gen_z2,gen_z3, eos_syntax=None, prev_z=None):
        """
        ...
        if eos_syntax is None:
            eos_syntax = {'response': 'EOS_M', 'user': 'EOS_U', 'bspan': 'EOS_Z2','constraint':'EOS_Z1','user_tag':'<split>','system':'EOS_Z2'}
        ...
        for i in range(batch_size):
            ...
            for key in turn_batch:
                ...
                if key in eos_syntax:
                    if key == 'bspan' or key == 'constraint' or key == 'user_tag' or key == 'system':
            ...
            if gen_z1:
                entry['generated_constraint'] = self.vocab.sentence_decode(gen_z1[i],eos='EOS_Z1')
            else:
                entry['generated_constraint'] = ''
            if gen_z2:
                entry['generated_user_tag'] = self.vocab.sentence_decode(gen_z2[i],eos='EOS_Z2')
            else:
                entry['generated_user_tag'] = ''
            if gen_z3:
                entry['generated_system'] = self.vocab.sentence_decode(gen_z3[i],eos = '<split>')
            else:
                entry['generated_system'] = ''
            entry['generated_bspan'] = self.vocab.sentence_decode(gen_z1[i],eos='EOS_Z2') +' EOS_Z2 '#+entry['generated_user_tag']
            #print('generated_constraint: ',entry['generated_constraint'] )

            #print('generated_user_tag: ',entry['generated_user_tag'])
            
            #print(entry['generated_bspan'])


            results.append(entry)
        write_header = False
        if not self.result_file:
            #time_string = str(time.strftime('%Y.%m.%d',time.localtime(time.time())))
            #file_name = time_string + cfg.result_path
            self.result_file = open(cfg.result_path, 'w', encoding='utf_8_sig')
            self.result_file.write(str(cfg))
            write_header = True

            # new dump 
            file_name = "./results/"+str(time.strftime('%Y-%m-%d-%H-%M-%S',time.localtime(time.time())))+"SPD-rl.csv"
            self.my_file = open(file_name,'w',encoding='utf_8_sig')
            



        field = ['dial_id', 'turn_num', 'user', 'generated_bspan', 'bspan','generated_constraint','constraint','generated_user_tag','user_tag','generated_system','system', 'generated_response', 'response', 'u_len','m_len', 'supervised']
        ...
        my_writer = csv.DictWriter(self.my_file,fieldnames=field)
...
class CamRest676Reader(_ReaderBase):
    def __init__(self):
        super().__init__()
        self.ev_dummy = CamRestEvaluator(result_path=cfg.result_path)
        self.ev_dummy.my_run_metrics()
    ...
    def _get_tokenized_data(self, raw_data, db_data, construct_vocab):
        ...
        for dial_id, dial in enumerate(raw_data):
            ...
            for turn in dial['dial']:
                ...
                future = []
                for item in turn['sys']["da"]:
                    try:
                        future.extend(word_tokenize(item))
                    except TypeError:
                        continue
                ...
                response,slot_list = self._replace_entity(turn['sys']['sent'], vk_map, constraint)
                
                ev = self.ev_dummy
                save_response = response
                
                _ , slot_list2 =  ev.clean(response,ifreader=True)
                # if len(slot_list2) > 0:
                #     print('response', response)
                #     #print('save response', save_response)
                #     print(slot_list2)
                
                response = word_tokenize(response) + ['EOS_M']
                future = slot_list+slot_list2+future

                #if i_turn == len(dial['dial']) and future == ['<split>']:
                ...
                if construct_vocab:
                    for word in user + response + constraint + requested+future:
    ...
    def _replace_entity(self, response, vk_map, constraint):
        slot_list = []
        response,count_post = re.subn('[cC][., ]*[bB][., ]*\d[., ]*\d[., ]*\w[., ]*\w', 'postcode_SLOT', response)
        response,count_phone = re.subn('\d{5}\s?\d{6}', 'phone_SLOT', response)
        if count_post>0:
            slot_list.append('postcode_SLOT')
        if count_phone>0:
            slot_list.append('phone_SLOT')
    ...
    def _get_encoded_data(self, tokenized_data):
        ...
        for dial in tokenized_data:
            ...
            for turn in dial:
                ...
                future = self.vocab.sentence_encode(turn['future'])
        ...
        file1 = open('./prepared_data.pkl','wb')
        pickle.dump(encoded_data,file1,protocol=0)  
        return encoded_data
    ...
    def _construct(self, data_json_path, db_json_path):
        ...
        #encoded_data = pickle.load(open('./prepared_data.pkl','rb'))
